  ...
  const debugModes: { mode: DebugMode; label: string; description: string }[] =
    [
      { mode: "off", label: "Off", description: "Normal terrain view" },
      {
        mode: "wireframe",
        label: "Wireframe",
        description: "Green wireframe mesh",
      },
      {
        mode: "heightmap",
        label: "Height Map",
        description: "Blue→Green→Red elevation",
      },
      { mode: "normals", label: "Normals", description: "RGB normal colors" },
      {
        mode: "vertex-dots",
        label: "Vertices",
        description: "Red vertex dots",
      },
      {
        mode: "watermap",
        label: "Water Map",
        description: "Water level overlay",
      },
    ];
  ...
  return (
    <div className="fixed right-4 top-4 z-50">
      ...
      >
        🔍 Debug Mesh{" "}
        {meshDebugMode !== "off" &&
          `(${debugModes.find((m) => m.mode === meshDebugMode)?.label})`}
      </button>
      ...
      {showUI && (
        <div className="w-80 rounded-lg border border-white/20 bg-black/90 p-4 text-white backdrop-blur-sm">
          ...
          <div className="mb-3 text-xs">
            ...
            <span
              className={`font-mono ${globeRef ? "text-green-400" : "text-red-400"}`}
            >
            ...
            {globeRef && (
              <span className="ml-2 text-gray-400">
                (
                {globeRef.geometry?.attributes.position?.count?.toLocaleString()}{" "}
                vertices)
              </span>
            )}
          </div>
          ...
          {stats && (
            <div className="space-y-2 text-sm">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>Deformed:</div>
                <div className="font-mono">
                  {stats.deformedVertices.toLocaleString()}
                </div>

                <div>With Water:</div>
                <div className="font-mono">
                  {stats.waterVertices.toLocaleString()}
                </div>

                <div>Height Range:</div>
                <div className="font-mono">
                  {stats.heightRange.min} to {stats.heightRange.max}
                </div>
          ...
          {meshDebugMode === "wireframe" && (
            <div className="mt-4 text-xs">
              ...
              <p className="mt-1 text-gray-300">
                Shows the deformed mesh structure in bright green. Try
                terraforming to see how the mesh changes!
              </p>
            </div>
          )}
          ...
          {meshDebugMode === "normals" && (
            <div className="mt-4 text-xs">
              ...
              <p className="mt-1 text-gray-300">
                RGB colors represent surface normals. Watch how terraforming
                changes the surface directions!
              </p>
            </div>
          )}
          ...
          {meshDebugMode === "vertex-dots" && (
            <div className="mt-4 text-xs">
              ...
              <p className="mt-1 text-gray-300">
                ...
                {stats &&
                  ` Displaying ~${Math.floor(stats.totalVertices / 10).toLocaleString()} of ${stats.totalVertices.toLocaleString()} vertices.`}
              </p>
            </div>
          )}
        </div>
