...
export function IdleOrientationTest() {
  ...
  useFrame((_state, _delta) => {
    ...
    const rotation = targetDeerObj.rotation
      ? new THREE.Vector3(...targetDeerObj.rotation)
    ...
    const deerMovement = (
      targetDeerObj as unknown as {
        deerMovement?: { isMoving?: boolean; state?: string };
      }
    ).deerMovement;
    const isIdle = deerMovement
      ? deerMovement.state === "idle" || !deerMovement.isMoving
      : "unknown";
    ...
    if (uprightAlignment < 0.8) {
      ...
      addResult(
        `   Upright alignment: ${uprightAlignment.toFixed(4)} (should be >0.8)`,
      );
      addResult(
      ...
      addResult(
        `   Surface normal: [${surfaceNormal.x.toFixed(3)}, ${surfaceNormal.y.toFixed(3)}, ${surfaceNormal.z.toFixed(3)}]`,
      );
      addResult(
        `   Deer up vector: [${deerUp.x.toFixed(3)}, ${deerUp.y.toFixed(3)}, ${deerUp.z.toFixed(3)}]`,
      );
      ...
      if (rotationMagnitude > 0.1) {
        addResult(
          `   🔍 Non-zero rotation detected (magnitude: ${rotationMagnitude.toFixed(4)})`,
        );

        // Check which axis has the problematic rotation
        if (Math.abs(rotation.x) > 0.05)
          addResult(
            `   ⚠️ X-axis rotation: ${((rotation.x * 180) / Math.PI).toFixed(1)}° (tilting forward/backward)`,
          );
        if (Math.abs(rotation.y) > 0.05)
          addResult(
            `   ⚠️ Y-axis rotation: ${((rotation.y * 180) / Math.PI).toFixed(1)}° (turning left/right)`,
          );
        if (Math.abs(rotation.z) > 0.05)
          addResult(
            `   ⚠️ Z-axis rotation: ${((rotation.z * 180) / Math.PI).toFixed(1)}° (rolling left/right)`,
          );
      }
    }
  });
  ...
  const testIdleBounceLogic = () => {
    ...
    lastMovementSpeed = Math.max(
      0,
      lastMovementSpeed - bounceDecayRate * mockDelta,
    );
    ...
    const bounceHeight =
      ...
      maxBounceHeight *
      Math.min(lastMovementSpeed / 1.5, 1.0);
    ...
    if (Math.abs(bounceHeight) > 0.01) {
      addResult(
        `  ❗ Bounce is active (${bounceHeight.toFixed(6)} > 0.01) - position will be modified`,
      );
      ...
      addResult(
        `  🔍 POTENTIAL ISSUE: Position modification during idle may not preserve orientation`,
      );
      addResult(
        `  🔧 Check if setTranslation() in DeerPhysics.tsx:194 affects rotation`,
      );
    } else {
      addResult(
        `  ✅ Bounce inactive (${bounceHeight.toFixed(6)} <= 0.01) - no position changes`,
      );
    }
  };
  ...
  const analyzeIdleStateIssue = () => {
    ...
    addResult(
      "1. During idle, deer still have bounce animation (lines 183-196)",
    );
    addResult(
      "2. When bounce height > 0.01, position is recalculated (line 191)",
    );
    addResult(
      "3. Code: adjustedPosition = currentPosition.clone().normalize().multiplyScalar(idealSurfaceDistance)",
    );
    addResult("4. This normalizes position but doesn't account for rotation!");
    addResult("");
    addResult("🔥 ROOT CAUSE HYPOTHESIS:");
    addResult(
      "The bounce animation during idle modifies position using setTranslation()",
    );
    addResult(
      "but doesn't maintain the deer's upright orientation relative to surface normal",
    );
    addResult("");
