...
export function AnimalOrientationTest() {
  ...
  const addTestResult = (message: string) => {
    setTestResults(prev => [...prev.slice(-10), `${logCounter.current++}: ${message}`]);
  };
  ...
  const testSurfaceNormals = () => {
    ...
    testPositions.forEach((pos, i) => {
      ...
      addTestResult(`Position ${i}: Normal accuracy = ${normalAccuracy.toFixed(4)} (should be ~1.0)`);
      addTestResult(`  Position: [${pos.x.toFixed(2)}, ${pos.y.toFixed(2)}, ${pos.z.toFixed(2)}]`);
      addTestResult(`  Normal: [${surfaceNormal.x.toFixed(4)}, ${surfaceNormal.y.toFixed(4)}, ${surfaceNormal.z.toFixed(4)}]`);
    });
  };
  ...
  const testInitialOrientation = () => {
    ...
    const testPosition = new THREE.Vector3(3, 3, 3).normalize().multiplyScalar(6.05);
    const surfaceNormal = testPosition.clone().normalize();
    
    addTestResult(`Test position: [${testPosition.x.toFixed(2)}, ${testPosition.y.toFixed(2)}, ${testPosition.z.toFixed(2)}]`);
    addTestResult(`Surface normal: [${surfaceNormal.x.toFixed(4)}, ${surfaceNormal.y.toFixed(4)}, ${surfaceNormal.z.toFixed(4)}]`);
    ...
    addTestResult(`World up dot surface normal: ${worldUp.dot(localUp).toFixed(4)}`);
    addTestResult(`Initial forward length: ${localForward.length().toFixed(4)}`);
    ...
    addTestResult(`Final quaternion: [${initialQuaternion.x.toFixed(4)}, ${initialQuaternion.y.toFixed(4)}, ${initialQuaternion.z.toFixed(4)}, ${initialQuaternion.w.toFixed(4)}]`);
    ...
    addTestResult(`Upright accuracy: ${uprightAccuracy.toFixed(4)} (should be close to 1.0)`);
  };
  ...
  const testLiveDeerOrientation = () => {
    ...
    deer.forEach((deerObj, i) => {
      ...
      const rotation = deerObj.rotation ? new THREE.Vector3(...deerObj.rotation) : new THREE.Vector3(0, 0, 0);
      const surfaceNormal = position.clone().normalize();
      
      addTestResult(`Deer ${i} at [${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)}]`);
      addTestResult(`  Rotation: [${rotation.x.toFixed(4)}, ${rotation.y.toFixed(4)}, ${rotation.z.toFixed(4)}]`);
      addTestResult(`  Surface normal: [${surfaceNormal.x.toFixed(4)}, ${surfaceNormal.y.toFixed(4)}, ${surfaceNormal.z.toFixed(4)}]`);
      ...
      addTestResult(`  Upright alignment: ${uprightAlignment.toFixed(4)} (1.0 = perfect, 0.0 = perpendicular)`);
      
      if (uprightAlignment < 0.8) {
        addTestResult(`  ⚠️ ISSUE: Deer ${i} is not properly upright relative to globe surface!`);
      }
    });
  };
  ...
  const testIdleStateOrientation = () => {
    ...
    deer.forEach((deerObj, i) => {
      const deerMovement = (deerObj as unknown as { deerMovement?: { isMoving?: boolean; state?: string } }).deerMovement;
      if (deerMovement) {
        ...
        if (isIdle) {
          ...
          addTestResult(`  Idle upright alignment: ${uprightAlignment.toFixed(4)}`);
          
          if (uprightAlignment < 0.8) {
            addTestResult(`  🔥 FOUND ISSUE: Idle deer ${i} losing upright orientation!`);
            addTestResult(`  🔍 Root cause: Idle state not maintaining surface normal alignment`);
          }
        }
      }
    });
