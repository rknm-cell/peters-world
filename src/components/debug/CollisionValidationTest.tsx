...
export function CollisionValidationTest() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      results.push({
        ...
        details: heightMapData
          ? `Generated ${heightMapData.resolution}x${heightMapData.resolution} map`
          : "Failed to generate",
      });
    } catch (error) {
      results.push({
    ...
    try {
      ...
      results.push({
        ...
        performance: endTime - startTime,
        details: normalMapData
          ? `Generated ${normalMapData.resolution}x${normalMapData.resolution} map`
          : "Failed to generate",
      });
    } catch (error) {
      results.push({
    ...
    try {
      ...
      for (let i = 0; i < testPositions.length - 1; i++) {
        const result = detector.checkMovement(
          testPositions[i]!,
          testPositions[i + 1]!,
        );
    ...
    try {
      ...
      results.push({
        ...
        performance: endTime - startTime,
        details: pathResult
          ? `Generated path with ${pathResult.path.length} points, confidence: ${(pathResult.confidence * 100).toFixed(1)}%`
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <button
          ...
          className="rounded-lg bg-blue-600 px-3 py-2 text-sm text-white shadow-lg hover:bg-blue-700"
  ...
  return (
    <div className="fixed left-4 top-4 z-50 max-h-96 max-w-md overflow-y-auto rounded-lg bg-black/90 p-4 text-white backdrop-blur-sm">
