...
export function WaterTerraformTool({ 
  ...
  const findVerticesInBrush = useCallback((intersectionPoint: THREE.Vector3, radius: number) => {
    const affectedVertices: Array<{ index: number; distance: number; vertex: typeof terrainVertices[0] }> = [];
  ...
  const applyWaterEffect = useCallback((intersectionPoint: THREE.Vector3) => {
    ...
    const affectedVertices = findVerticesInBrush(intersectionPoint, brushSize);
    ...
    affectedVertices.forEach(({ index, distance, vertex }) => {
      ...
      switch (mode) {
        case 'add':
          ...
          if (surroundingVertices.length > 1) {
            const totalHeight = surroundingVertices.reduce((sum, sv) => sum + sv.vertex.height, 0);
        ...
        case 'level':
          ...
          if (nearbyVertices.length > 1) {
            const averageWater = nearbyVertices.reduce((sum, nv) => sum + nv.vertex.waterLevel, 0) / nearbyVertices.length;
            newWaterLevel = THREE.MathUtils.lerp(vertex.waterLevel, averageWater, effectStrength);
          }
          break;
      }
      ...
      if (Math.abs(newWaterLevel - vertex.waterLevel) > 0.01) {
        ...
        console.log(`💧 Water terraforming: Vertex ${index} water level ${vertex.waterLevel.toFixed(2)} → ${newWaterLevel.toFixed(2)}`);
      }
    });
  }, [brushSize, flowRate, mode, findVerticesInBrush, updateTerrainVertex]);
  
  // Handle mouse/touch interaction
  const handlePointerInteraction = useCallback((event: MouseEvent | TouchEvent) => {
    if (!isActive) return;
    ...
    mousePositionRef.current.y = -((clientY - rect.top) / rect.height) * 2 + 1;
    ...
    const terrainMesh = scene.children.find(child => 
      child instanceof THREE.Mesh && 
      child.userData?.isTerrainMesh === true
    ) as THREE.Mesh;
  ...
  useFrame(() => {
    ...
    if (isActive) {
      ...
      canvas.style.cursor = mode === 'add' ? 'crosshair' : 
                           mode === 'remove' ? 'grab' : 'move';
    } else {
  ...
  return (
    <group>
      ...
      <mesh>
        ...
        <meshBasicMaterial 
          color={mode === 'add' ? 0x0099ff : mode === 'remove' ? 0xff6600 : 0x00ff00}
          transparent
          opacity={0.3}
