...
export function TreeLifecycleManager() {
  ...
  useEffect(() => {
    console.log(
      `🕒 TreeLifecycleManager starting with ${TREE_LIFECYCLE_CONFIG.checkInterval / 1000}s interval`,
    );
    ...
    const scheduleNextCheck = () => {
      ...
      const intervalWithJitter =
        TREE_LIFECYCLE_CONFIG.checkInterval * (1 + jitter);

      timeoutId = setTimeout(() => {
        if (!isActive) return;

        console.log(
          "⏰ TreeLifecycleManager tick - running lifecycle updates...",
        );
