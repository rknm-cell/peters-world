...
function WolfPhysicsComponent({ objectId, position, type }: WolfPhysicsProps) {
  ...
  const [huntingOrientation, setHuntingOrientation] =
  ...
  const MOVEMENT_SPEED =
    WOLF_CONFIG.movement.moveSpeed.min +
    Math.random() *
  ...
  const findNearbyDeer = (wolfPosition: THREE.Vector3) => {
    const deerObjects = otherAnimalsFiltered.filter(
  ...
  useFrame((state, delta) => {
    ...
    if (isIdle) {
      ...
      const maxIdleDuration =
        IDLE_DURATION.min +
        Math.random() * (IDLE_DURATION.max - IDLE_DURATION.min);
      ...
      if (localForward.length() < 0.1) {
        ...
        localForward
          ...
          .sub(
            surfaceNormal
              .clone()
              .multiplyScalar(worldForward.dot(surfaceNormal)),
          )
          .normalize();
      }
      ...
      if (
        orientationDifference > ORIENTATION_DRIFT_THRESHOLD &&
        !isUserInteracting
      ) {
        ...
        if (orientationDifference > 0.2) {
          ...
          console.log(
            `🐺 Wolf ${objectId}: Correcting orientation drift (${((orientationDifference * 180) / Math.PI).toFixed(1)}°)`,
          );
        }
      }
    ...
    if (isHunting) {
      ...
      if (huntingDuration >= HUNTING_DURATION) {
        ...
        setTarget(null); // Force new target generation
        console.log(
          `🐺 Wolf ${objectId}: Hunting timeout, returning to wandering`,
        );
      } else if (huntingTargetId) {
        // Check if target deer still exists
        const store = useWorldStore.getState();
        const targetDeer = store.objects.find(
          (obj) => obj.id === huntingTargetId,
        );
      ...
      if (huntingOrientation) {
        ...
        const worldUp = new THREE.Vector3(0, 1, 0);
        const localForward = worldUp.clone().cross(surfaceNormal).normalize();

        // Handle edge case where surface normal is parallel to world up
        if (localForward.length() < 0.1) {
          localForward.set(1, 0, 0).cross(surfaceNormal).normalize();
        }
      ...
      if (
        ...
        if (orientationDifference > 0.2) {
          ...
          console.log(
            `🐺 Wolf ${objectId}: Correcting orientation drift while hunting using ${orientationType} orientation (${((orientationDifference * 180) / Math.PI).toFixed(1)}°)`,
          );
        }
      }
    ...
    if (
      nearbyDeer &&
      !isHunting &&
      Math.random() < WOLF_CONFIG.hunting.huntingProbability
    ) {
      ...
      if (distanceToDeer <= DEER_DETECTION_RADIUS) {
        ...
        setIsIdle(false);
        console.log(
          `🐺 Wolf ${objectId}: Starting to hunt deer at distance ${distanceToDeer.toFixed(2)}`,
        );
      }
    }
    ...
    if (!isHunting || !huntingTargetId) {
      ...
      if (needsNewTarget) {
        ...
        if (targetReached && target) {
          console.log(
            `🐺 Wolf ${objectId}: Reached target at distance ${distanceToTarget.toFixed(2)}`,
          );
        ...
        if (newTarget) {
          setTarget(newTarget);
          console.log(
            `🐺 Wolf ${objectId}: New target set at distance ${currentPosition.distanceTo(newTarget).toFixed(2)}`,
          );
        }
      }
    }
    ...
    if (target && !isIdle) {
      ...
      const currentSpeed = isHunting
        ? WOLF_CONFIG.hunting.chaseSpeed
        : MOVEMENT_SPEED;
      ...
      console.log(
        `🐺 Wolf ${objectId}: Checking movement from ${currentPosition.x.toFixed(2)}, ${currentPosition.y.toFixed(2)}, ${currentPosition.z.toFixed(2)} to ${targetPosition.x.toFixed(2)}, ${targetPosition.y.toFixed(2)}, ${targetPosition.z.toFixed(2)}`,
      );
      ...
      console.log(
        `🐺 Wolf ${objectId}: Collision result: canMove=${terrainCollision.canMove}, groundHeight=${terrainCollision.groundHeight.toFixed(2)}, isWater=${terrainCollision.isWater}`,
      );
      ...
      if (!terrainCollision.canMove) {
        ...
        console.log(
          `🐺 Wolf ${objectId}: Enhanced validation result: isValid=${enhancedValidation.isValid}, confidence=${(enhancedValidation.confidence * 100).toFixed(1)}%`,
        );
      }
      ...
      if (!terrainCollision.canMove) {
        ...
        console.log(
          `🐺 Wolf ${objectId}: Traditional collision detection blocked movement`,
          {
            reason: blockReason,
            ...
            slopeAngle:
              ((terrainCollision.slopeAngle * 180) / Math.PI).toFixed(1) + "°",
        ...
        if (terrainCollision.isBuildingBlocked) {
          console.log(
            `🐺 Wolf ${objectId}: Blocked by building (${terrainCollision.blockedByBuilding}), generating new target`,
          );
          setTarget(null); // Force new target generation
          ...
          console.log(
            `🐺 Wolf ${objectId}: Using enhanced alternative path point`,
          );
        } else if (terrainCollision.adjustedPosition) {
          targetPosition = terrainCollision.adjustedPosition;
          console.log(
            `🐺 Wolf ${objectId}: Using traditional adjusted position`,
          );
        } else {
          // No alternative available, generate new target
          setTarget(null);
          console.log(
            `🐺 Wolf ${objectId}: Generating new target due to blocked movement`,
          );
          return; // Skip movement this frame
        }
      } else {
        ...
        console.log(
          `🐺 Wolf ${objectId}: Movement allowed, positioning at height ${terrainGroundHeight.toFixed(2)}`,
        );
      }
      ...
      const bounceHeight =
        Math.sin(bouncePhase.current) *
        maxBounceHeight *
        Math.min(currentMovementSpeed / currentSpeed, 1.0);

      // Apply bounce to final position
      const bounceOffset = targetPosition
        .clone()
        .normalize()
        .multiplyScalar(bounceHeight);
      ...
      queueDeerTransformUpdate(
        ...
        () => {
          ...
          if (actualMovement.length() > 0.01) {
            const { positionVec, directionVec, surfaceNormal } =
              extractMovementVectors(currentPosition, actualMovement);
   ...
   */
  function generateWanderingTarget(
    ...
    const baseDistance =
      TARGET_DISTANCE.min +
      Math.random() * (TARGET_DISTANCE.max - TARGET_DISTANCE.min);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const terrainHeight = terrainHeightMapGenerator.sampleHeight(
        targetDirection.clone().multiplyScalar(6.0),
      );
      ...
      if (pathValidation.isValid && pathValidation.confidence > 0.6) {
        console.log(
          `🐺 Wolf ${objectId}: Enhanced pathfinding found valid target after ${attempt + 1} attempts (confidence: ${(pathValidation.confidence * 100).toFixed(1)}%)`,
        );
        ...
        if (terrainCollision.canMove) {
          console.log(
            `🐺 Wolf ${objectId}: Fallback collision detection found target after ${attempt + 1} attempts`,
          );
          return candidateTarget;
        }
      }
      ...
      if (attempt % 5 === 0) {
        ...
        console.log(
          `🐺 Wolf ${objectId}: Target attempt ${attempt + 1} rejected - ${pathValidation.reason ?? "Unknown reason"} (confidence: ${(pathValidation.confidence * 100).toFixed(1)}%)`,
        );
      }
    }

    console.log(
      `🐺 Wolf ${objectId}: Could not find valid wandering target after ${maxAttempts} attempts`,
    );
