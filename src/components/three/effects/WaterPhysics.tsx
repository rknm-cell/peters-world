...
export function WaterPhysics({ 
  ...
  const waterDataRef = useRef<Float32Array>(new Float32Array(resolution * resolution));
  const velocityDataRef = useRef<Float32Array>(new Float32Array(resolution * resolution * 2));
  const previousWaterDataRef = useRef<Float32Array>(new Float32Array(resolution * resolution));
  ...
  const simulateWaterPhysics = () => {
    if (!waterDataRef.current || !velocityDataRef.current || !previousWaterDataRef.current) {
      return;
    }
    ...
    for (let y = step; y < resolution - step; y += step) {
      for (let x = step; x < resolution - step; x += step) {
        ...
        const neighborHeights = [
          terrainVertices[Math.min(terrainIndex - 1, terrainVertices.length - 1)]?.height ?? 0,
          terrainVertices[Math.min(terrainIndex + 1, terrainVertices.length - 1)]?.height ?? 0,
          terrainVertices[Math.min(terrainIndex - resolution, terrainVertices.length - 1)]?.height ?? 0,
          terrainVertices[Math.min(terrainIndex + resolution, terrainVertices.length - 1)]?.height ?? 0,
        ];
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const heightDiff = (currentHeight + currentWater) - (neighborHeight + (neighborWater ?? 0));
          
          if (heightDiff > 0.1) { // Only flow if significant height difference
        ...
        for (let i = 0; i < 4; i++) {
          const neighborIndex = i === 0 ? index - 1 :
                               i === 1 ? index + 1 :
                               i === 2 ? index - resolution :
                               index + resolution;
  ...
  const updateTerrainWaterLevels = () => {
    ...
    for (let i = 0; i < Math.min(waterData.length, terrainVertices.length); i++) {
      const waterLevel = waterData[i];
      const vertex = terrainVertices[i];
      
      if (vertex && waterLevel !== undefined && Math.abs(vertex.waterLevel - waterLevel) > 0.01) {
  ...
  useFrame((state, _delta) => {
    ...
    if (frameCountRef.current % 120 === 0) { // Every 120 frames (~2fps at 60fps)
