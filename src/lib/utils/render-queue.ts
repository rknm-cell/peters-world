...
const PRIORITY_ORDER: Array<"high" | "normal" | "low"> = [
  "high",
  "normal",
  "low",
];
...
export function useRenderQueue(config: Partial<RenderQueueConfig> = {}) {
  ...
  const getQueueStats = useCallback(() => {
    ...
    return {
      ...
      highPriority: Array.from(queue.values()).filter(
        (u) => u.priority === "high",
      ).length,
      normalPriority: Array.from(queue.values()).filter(
        (u) => u.priority === "normal",
      ).length,
      lowPriority: Array.from(queue.values()).filter(
        (u) => u.priority === "low",
      ).length,
      oldestItem:
        queue.size > 0
          ? Math.min(
              ...Array.from(queue.values()).map((u) => now - u.timestamp),
            )
          : 0,
    };
  }, []);
...
export function useDeerRenderQueue(isUserInteracting?: boolean) {
   ...
   */
  const cancelDeerUpdates = useCallback(
    (deerId: string) => {
