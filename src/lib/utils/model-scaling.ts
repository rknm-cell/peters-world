...
export function getTargetHeight(options: ScalingOptions): number {
  ...
  switch (objectType) {
    case "tree":
      if (lifecycleStage) {
        targetHeight =
          MODEL_SCALING.targetHeights.trees[lifecycleStage] ??
          MODEL_SCALING.targetHeights.trees.adult;
      } else {
    ...
    case "animal":
      const animalKey = modelType.replace(
        "animals/",
        "",
      ) as keyof typeof MODEL_SCALING.targetHeights.animals;
    ...
    case "decoration":
      const decorationKey =
        modelType as keyof typeof MODEL_SCALING.targetHeights.decorations;
      targetHeight =
        MODEL_SCALING.targetHeights.decorations[decorationKey] ??
        MODEL_SCALING.targetHeights.decorations.rock;
      break;

    case "structure":
      const structureKey =
        modelType as keyof typeof MODEL_SCALING.targetHeights.structures;
      targetHeight =
        MODEL_SCALING.targetHeights.structures[structureKey] ?? 1.0;
      break;
 ...
 */
export function applyStandardizedScaling(
  scene: THREE.Object3D,
 ...
 */
export function applySpecialScaling(
  scene: THREE.Object3D,
  modelType: string,
  lifecycleStage?: TreeLifecycleStage,
): number {
